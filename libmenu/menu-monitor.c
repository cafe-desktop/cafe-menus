...
static MenuMonitorNotify* cafe_menu_monitor_notify_ref(MenuMonitorNotify* notify);
static void cafe_menu_monitor_notify_unref(MenuMonitorNotify* notify);
...
static void invoke_notifies(MenuMonitor* monitor, MenuMonitorEvent  event, const char* path)
{
  ...
  while (tmp != NULL)
      ...
      if (notify->notify_func)
	{
      ...
      cafe_menu_monitor_notify_unref(notify);
  ...
  while (tmp != NULL)
    {
      ...
      cafe_menu_monitor_ref(event_info->monitor);
...
static MenuMonitor* lookup_monitor(const char* path, gboolean is_directory)
{
  ...
  if (retval == NULL)
  ...
  else
    {
      ...
      return cafe_menu_monitor_ref(retval);
    }
}

MenuMonitor* cafe_menu_monitor_file_get(const char* path)
{
...
MenuMonitor* cafe_menu_monitor_ref(MenuMonitor* monitor)
{
...
void cafe_menu_monitor_notify_remove(MenuMonitor* monitor, MenuMonitorNotifyFunc notify_func, gpointer user_data)
{
	GSList* tmp = monitor->notifies;
